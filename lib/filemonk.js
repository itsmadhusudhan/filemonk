(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.FileMonk = {}));
  ...
  function createListener() {
    ...
    const emit = (payload) => {
      listeners.filter((listener) => listener.event === payload.type).map((listener) => listener.cb.bind(listener)).forEach((cb) => cb({
        type: payload.type,
        data: payload.data
      }));
    };
  ...
  var FileItemStatus = /* @__PURE__ */ ((FileItemStatus2) => {
    FileItemStatus2["IDLE"] = "IDLE";
    FileItemStatus2["IN_QUEUE"] = "IN_QUEUE";
    FileItemStatus2["UPLOADING"] = "UPLOADING";
    FileItemStatus2["UPLOADED"] = "UPLOADED";
    FileItemStatus2["FAILED"] = "FAILED";
    FileItemStatus2["CANCELLED"] = "CANCELLED";
    return FileItemStatus2;
  })(FileItemStatus || {});
  var FileItemEvents = /* @__PURE__ */ ((FileItemEvents2) => {
    FileItemEvents2["ON_ITEM_UPDATED"] = "ON_ITEM_UPDATED";
    FileItemEvents2["ON_FILE_PROCESS_START"] = "ON_FILE_PROCESS_START";
    FileItemEvents2["ON_FILE_PROCESS_PROGRESS"] = "ON_FILE_PROCESS_PROGRESS";
    FileItemEvents2["ON_FILE_PROCESS_COMPLETE"] = "ON_FILE_PROCESS_COMPLETE";
    FileItemEvents2["ON_FILE_PROCESS_CANCELLED"] = "ON_FILE_PROCESS_CANCELLED";
    FileItemEvents2["ON_FILE_PROCESS_FAILED"] = "ON_FILE_PROCESS_FAILED";
    return FileItemEvents2;
  })(FileItemEvents || {});
  var StoreActions = /* @__PURE__ */ ((StoreActions2) => {
  })(StoreActions || {});

  const createItem = (file, server) => {
  ...
  const createQueryHandlers = (store) => {
    return {
      [queries.GET_IDLE_ITEMS]: () => {
        return store.getState().items.filter((item) => item.status.get() === FileItemStatus.IDLE);
      },
      [queries.GET_MAX_PARALLEL_UPLOADS_ITEMS]: () => {
        return store.getState().maxParallelUploads;
      },
      [queries.GET_ITEMS_BY_STATUS]: (status) => {
      ...
      [queries.GET_PROCESSED_ITEMS]: () => {
        return store.getState().items.filter((item) => {
          return item.status.get() !== FileItemStatus.IDLE && item.status.get() !== FileItemStatus.IN_QUEUE && item.status.get() !== FileItemStatus.UPLOADING;
        });
      }
    };
  };
  ...
  const createHandlers = (store) => {
    ...
    const processFile = ({ query }) => {
      const item = typeof query === "string" ? store.getState().items.find((i) => i.id.get() === query) : query;
      if (!item) {
      ...
      const maxParallelUploads = store.query(queries.GET_MAX_PARALLEL_UPLOADS_ITEMS);
      const currentUploads = store.query(queries.GET_ITEMS_BY_STATUS, "UPLOADING");
      ...
      const processNext = () => {
        ...
        const itemReference = store.getState().items.find((i) => i.id.get() === queueEntry);
        if (!itemReference) {
          processNext();
          return;
        }
      ...
      item.subscribeOnce(FileItemEvents.ON_FILE_PROCESS_COMPLETE, () => {
        store.dispatch("DID_PROCESSING_COMPLETE", { item });
        processNext();
  ...
  const createStore = (listener, config) => {
    let state = {
      items: [],
      processingQueue: [],
  ...
  const createApp = (config = { maxParallelUploads: 1, name: "fileMonk" }) => {
    const listener = createListener();
