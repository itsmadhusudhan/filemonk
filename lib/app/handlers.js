...
// FIXME: return type
var createHandlers = function (store) {
    var createFileItem = function (data) {
        var serverConfig = store.query(queries_1.queries.GET_SERVER_CONFIG);
        var fileItem = (0, createItem_1.createItem)(data.file, __assign(__assign({}, data.server), { config: __assign(__assign({}, serverConfig), data.server.config) }), data.context);
        fileItem.subscribe("ON_ITEM_UPDATED", function () {
            store.setState({});
        });
        store.dispatch("DID_CREATE_ITEM", (0, transformFileItem_1.transformFileItem)(fileItem));
        fileItem.subscribe("ON_FILE_PROCESS_START", function () {
            store.dispatch("ON_FILE_PROCESS_START", (0, transformFileItem_1.transformFileItem)(fileItem));
        });
        store.setState({
    ...
    var processFile = function (_a) {
        ...
        var maxParallelUploads = store.query(queries_1.queries.GET_MAX_PARALLEL_UPLOADS_ITEMS);
        var currentUploads = store.query(queries_1.queries.GET_ITEMS_BY_STATUS, "UPLOADING");
        ...
        item.subscribeOnce("ON_FILE_PROCESS_COMPLETE", function (r) {
            store.dispatch("DID_PROCESSING_ITEM_COMPLETE", {
                item: (0, transformFileItem_1.transformFileItem)(item),
                status: "SUCCESS",
                response: r.data,
            });
            processNext();
            var items = store.query(queries_1.queries.GET_PROCESSED_ITEMS);
